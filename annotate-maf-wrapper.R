...
parser$add_argument('-s', '--sample-mapping', required = T,
                    help = 'Tab-separated file with header where first column contains sample ID and second column path to FACETS Rdata output file.')
...
output_maf = ifelse(
    length(common_samples) < 10,
    lapply(common_samples, annotate_sample),
    mclapply(common_samples, annotate_sample, mc.cores = detectCores())
)
